# GraphSAGE Recommender System - MVP Architecture

## Executive Summary

This document outlines the MVP architecture for a user-facing GraphSAGE-based movie recommender system. The system provides personalized movie recommendations using Graph Neural Networks (GNN), supporting both cold-start scenarios (demographic-only) and warm-start scenarios (with user ratings).
...
- **Recommendations & Rating Panel**: Display personalized movie recommendations with inline rating interface (1-5 stars)
...
- `GET /api/recommendations/{user_id}`: Get personalized recommendations (handles both cold-start and warm-start automatically)
  - Query params: `n` (default: 10), `exclude_low_rated` (default: true)
  - Note: When `exclude_low_rated=true`, filters out movies rated ≤2 stars (poorly rated)
...
- **Prediction Service**: Generate rating predictions and top-N recommendations
...
6. Filter out poorly-rated items (rating ≤2 stars) to avoid recommending movies the user disliked
...
- Store models in `models/current/` directory
- Keep single production model
...
**Purpose**: Train and update the GraphSAGE model
...
7. Recommendation service computes recommendations using demographic similarity
8. FastAPI returns top-10 movie recommendations
9. Streamlit displays recommendations page with inline rating interface for each movie

### Rating Addition Flow (Inline with Recommendations)

1. User rates a recommended movie (1-5 stars) directly in the recommendations list
...
7. Streamlit auto-refreshes recommendations to show updated list
8. If rating ≤2 (poorly rated), the movie is removed from future recommendations; if rating ≥3, it may still appear as a recommendation

### Recommendation Refresh Flow

1. User clicks "Refresh Recommendations" or rating triggers auto-refresh
...
5. Recommendation service generates recommendations using rating history
6. FastAPI returns updated top-10 recommendations (excluding poorly-rated movies with rating ≤2)
7. Streamlit displays refreshed recommendations with inline rating widgets
...
│   │   │   └── 2_recommendations.py        # Recommendations display with inline rating
│   │   │
...
3. **Data Privacy**: No PII collection beyond demographics
...
2. Develop training pipeline and train initial model
3. Implement API endpoints
4. Build inference engine with graph management
...
This MVP architecture provides a solid foundation for a user-facing GraphSAGE recommender system. It balances simplicity for demonstration with extensibility for later enhancements. The modular design allows each component to be developed, tested, and deployed independently.

---

**Document Version**: 1.0 (MVP)  
**Last Updated**: 2026-01-30  
**Status**: MVP-Focused Architecture
